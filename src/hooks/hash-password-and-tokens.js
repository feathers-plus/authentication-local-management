...
function hashPasswordAndTokens(password, verifyToken, resetToken, resetShortToken) {
  return async (context) => {
    const context1 = await hashPassword({ passwordField: password || 'password'})(context);
    ...
    return await hashPassword({ passwordField: resetShortToken || 'resetShortToken'})(context3);
  };
}
